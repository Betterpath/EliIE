...
def txt2matrix_fortrain(ann_dir,mytrain,tag_included,filename,curpath):
    ...
    print 'There are '+ str(len(txt_files))+' in total!'
...
# txt2matrix_fortrain('training','output/relation.matrix',['Observation','Condition','Drug','Procedure_Device'],)
...
def generate_XML(crfresult_input,NERxml_output):
    ...
    print >>NERxml_output,'<?xml version="1.0" encoding="UTF-8"?>'
...
def main():
    ...
    matrix_dir='output/test_'+filename+  '.matrix'
    mymatrix=codecs.open(matrix_dir,'w')
    crf_result_dir='output/test_'+filename+'_crf.result'
